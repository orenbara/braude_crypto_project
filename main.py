    ...
    while not success:
        try:
            ...
            if rsa_verify(rsa_public_key, str(encrypted_key).encode(), signature):
                ...
                if salsa_key == decrypted_key:
                    print("Decryption successful. Recovered key matches the original key.")
                    success = True
                else:
                    print("Decryption failed. Recovered key does not match the original key.")
                    continue
